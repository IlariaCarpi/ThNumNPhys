...
NP = 500  # number of moves to find parameters
NX = 10000  # number of particles' moves
step = 1.0  # algorithm step for random walk of particles
step_frac = 0.3 # parameter to calculate the adaptive step in parameters
...
for l in range(0,NP):
  ...
  if (l==0):
    ...
    if (list_Eall[l-1]>E_mean or abs(list_Eall[l-1]/E_mean)>np.random.rand()):     
    ...
    # Adattamento dello step ogni 'n_agg_par' iterazioni
    if l > 0 and l % n_agg_par == 0:
      acc_ratio = N_accept_param / N_total_param
      if acc_ratio < 0.5:
        step_frac *= 0.9  # riduci passo
      elif acc_ratio > 0.8:
        step_frac *= 1.1  # aumenta passo
      step_frac = min(max(step_frac, 1e-3), 1.0)  # limiti di sicurezza
      N_accept_param = 0  # reset counters
      N_total_param = 0
